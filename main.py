...
async function openAddPurch(){
  ...
  <div style="background:var(--bg2);border:1px solid var(--border);border-radius:16px;width:100%;max-width:900px;max-height:95vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,.7)">
    ...
    <div style="padding:14px 22px;border-bottom:1px solid var(--border);background:rgba(255,255,255,.02)">
...
async function addNewToPurchCart(){
  ...
  if(qty<=0){toast('الكمية يجب أن تكون أكبر من صفر','error');return;}
  // Check if product with same name or barcode exists in current list
  let prod = purchProds.find(p => (bar && p.barcode === bar) || p.name === name);
  if(prod) {
    const exist=purchCart.find(c=>c.id===prod.id);
    if(exist){exist.quantity+=qty;exist.total=exist.quantity*exist.cost_price;}
    else purchCart.push({id:prod.id,name:prod.name,category:prod.category,quantity:qty,cost_price:cost||prod.cost_price,sell_price:sell||prod.selling_price,total:qty*(cost||prod.cost_price),isNew:false});
  } else {
    ...
    purchCart.push({id:r.data?.id,name,category:cat,quantity:qty,cost_price:cost,sell_price:sell,total:qty*cost,isNew:true,isJustCreated:true});
  }
  renderPurchCart();
...
function newMobilePurchase(){
  omo('➕ فاتورة مشتريات جديدة',`<div class="fg">
    <div class="ff full"><label>المورد *</label><input id="psupp" placeholder="اسم المورد"></div>
    <div class="ff"><label>الإجمالي *</label><input id="ptotal" type="number" placeholder="0"></div>
    <div class="ff"><label>المدفوع</label><input id="ppaid" type="number" placeholder="0"></div>
...
function addPurItem(){
  ...
  const html=`<div id="${id}" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr auto;gap:5px;margin-bottom:10px">
    <input placeholder="اسم المنتج" class="pi-name">
    <input type="number" placeholder="الكمية" class="pi-qty">
    <input type="number" placeholder="السعر" class="pi-price">
    <span class="pi-total">0</span>
    <button class="btn btn-danger btn-sm" onclick="document.getElementById('${id}').remove()">🗑️</button>
  </div>`;
  cont.insertAdjacentHTML('beforeend',html);
  
  const el=document.getElementById(id);
  el.querySelector('.pi-qty').oninput=el.querySelector('.pi-price').oninput=()=>{
    const q=parseFloat(el.querySelector('.pi-qty').value)||0;
    const p=parseFloat(el.querySelector('.pi-price').value)||0;
    el.querySelector('.pi-total').textContent=fmt(q*p);
  };
}

async function savePurInv(){
  const items=Array.from(document.querySelectorAll('[id^="pi-"]')).map(el=>({
    product_id:0,
    quantity:parseFloat(el.querySelector('.pi-qty').value)||0,
    unit_price:parseFloat(el.querySelector('.pi-price').value)||0
  }));
  
  const data={
    supplier_id:0,
    items,
    total_amount:parseFloat(document.getElementById('ptotal').value)||0,
    paid_amount:parseFloat(document.getElementById('ppaid').value)||0
  };
  
  const r=await api('/api/mobile/purchases/add','POST',data);
  if(r.success){cmo();toast(r.message);mobile('purchases');}else toast(r.message,'error');
}

async async function delPurInv(id){
  if(!confirm('حذف الفاتورة؟'))return;
  const r = await api(`/api/purchases/${id}`,'DELETE');
  if(r.success){toast(r.message);mobile('purchases');}
  else toast(r.message || 'خطأ في الحذف','error');
}

async function viewPurInv(id,inv){
  const r = await api(`/api/purchases/${id}`);
  if(!r.success){
    omo('📋 تفاصيل الفاتورة: '+inv, `<div style="padding:20px;color:var(--danger)">خطأ في تحميل الفاتورة</div>`);
    return;
  }
  const {purchase, details} = r.data;
  ...
  `).join('');
  omo('📋 تفاصيل الفاتورة: '+purchase.invoice_number,`
    <div style="padding:16px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
        <div><strong>المورد:</strong> ${purchase.supplier_name||'—'}</div>
        <div><strong>التاريخ:</strong> ${purchase.purchase_date}</div>
      </div>
      <div style="background:var(--bg3);border:1px solid var(--border);border-radius:10px;overflow-x:auto">
        <table style="width:100%;border-collapse:collapse">
          <tr style="background:var(--bg2)"><th>المنتج</th><th>الكمية</th><th>سعر الوحدة</th><th>الإجمالي</th></tr>
          ${rows}
        </table>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:14px">
        <div><strong>الإجمالي:</strong> ${fmt(purchase.total_amount)}</div>
        <div><strong>المدفوع:</strong> ${fmt(purchase.paid_amount)}</div>
        <div><strong>المتبقي:</strong> ${fmt(purchase.remaining_amount)}</div>
      </div>
    </div>
  `,`<button class="btn btn-ghost" onclick="cmo()">إغلاق</button>`);
}
...
async function delDebt(id){
  ...
  const r = await api(`/api/mobile/debts/${id}`,'DELETE');
  if(r.success){toast(r.message);mobile('debts');}
  else toast(r.message || 'خطأ في الحذف','error');
}
...
def init_db():
    ...
    tables = [
        ...
        "CREATE TABLE IF NOT EXISTS purchases (id INTEGER PRIMARY KEY AUTOINCREMENT, invoice_number TEXT UNIQUE NOT NULL, supplier_id INTEGER, total_amount REAL NOT NULL, paid_amount REAL NOT NULL, remaining_amount REAL NOT NULL, purchase_date TEXT NOT NULL, user_id INTEGER NOT NULL, created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
...
def add_product():
    ...
    try:
        ...
        db.commit()
        db.close()
        return ok({"id": pid}, "تم إضافة المنتج")
...
def add_purchase():
    ...
    try:
        ...
        try:
            import re
            last_inv = last['invoice_number']
            match = re.search(r'(\d+)$', last_inv)
...
def pay_mobile_debt(sale_id):
    try:
        ...
        if not sale or pay_amount <= 0 or pay_amount > sale['remaining_amount']:
            conn.close()
        ...
        cursor.execute("""
            ...
            WHERE id=?
        """, (pay_amount, pay_amount, sale_id))
        
        today = date.today().isoformat()
        cursor.execute("""
            INSERT INTO payments (type, reference_id, amount, payment_date, payment_method, user_id)
            VALUES (?, ?, ?, ?, ?, ?)
        """, ('sale', sale_id, pay_amount, today, 'نقدي', session['user_id']))
...
def get_mobile_cash():
    try:
        ...
        query = """
            SELECT 
                CASE WHEN p.type='sale' THEN 'مبيعات' WHEN p.type='purchase' THEN 'مشتريات' ELSE p.type END as type,
                COALESCE(s.invoice_number, pur.invoice_number, '') as invoice_number,
                p.amount, p.payment_date as trans_date, u.username
            FROM payments p
            LEFT JOIN sales s ON p.type='sale' AND p.reference_id=s.id
            LEFT JOIN purchases pur ON p.type='purchase' AND p.reference_id=pur.id
            LEFT JOIN users u ON p.user_id = u.id
            WHERE p.payment_date BETWEEN ? AND ?
            AND (
                (p.type='sale' AND p.reference_id IN (
                    SELECT sale_id FROM sale_details sd
                    JOIN products pr ON sd.product_id = pr.id
                    WHERE pr.category = 'موبايلات'
                )) OR
                (p.type='purchase' AND p.reference_id IN (
